...
def predict(model, im):
    ...
    dict_p_vals = dict(zip(model.get_labels(),p_vals[0]))
    if VERBOSE:
        print chr(65+int(prediction)), max(probability)
        #print probability

    #return chr(65+int(prediction[0])), str(max(probability[1].values())), probability[1]
...
def break_captcha(model, letter1_algo, letter2_algo, letter3_algo, parent=None):
    ...
    if parent:
        ...
        print prediction1, max_score1, prediction2, max_score2, prediction3, max_score3, dico1, dico2, dico3
